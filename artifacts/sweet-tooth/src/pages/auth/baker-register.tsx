...
export default function BakerRegister() {
  ...
  return <div className="min-h-screen bg-background flex items-center justify-center p-4"><form onSubmit={submit} className="w-full max-w-md p-8 border border-border rounded-2xl bg-card shadow-lg space-y-5">
    ...
    <div className="grid gap-3"><input required placeholder="Bakery name" value={form.businessName} onChange={update("businessName")} className="px-4 py-3 border border-border rounded-md bg-background" /><input required placeholder="Your name" value={form.ownerName} onChange={update("ownerName")} className="px-4 py-3 border border-border rounded-md bg-background" /><input required placeholder="City" value={form.city} onChange={update("city")} className="px-4 py-3 border border-border rounded-md bg-background" /><input required placeholder="WhatsApp number" value={form.whatsappNumber} onChange={update("whatsappNumber")} className="px-4 py-3 border border-border rounded-md bg-background" /><input required type="email" placeholder="Email" autoComplete="email" value={form.email} onChange={update("email")} className="px-4 py-3 border border-border rounded-md bg-background" /><input required minLength={6} type="password" placeholder="Password (6+ characters)" autoComplete="new-password" value={form.password} onChange={update("password")} className="px-4 py-3 border border-border rounded-md bg-background" /></div>
